...
class Composer extends ComposerBase{
  ...
  items:[
    Config(Container, { style: "padding: 8px;background-color:" + config.adapter.getColor(),
      items:[
        Config(DisplayField, { value: this.getComposerTitle(config.adapter), ui: ConfigUtils.asString( null), style: "color:#fff;font-weight:bold;"}),
    ...
    Config(Container, { itemId:  Composer.EDITOR_PANEL,
               ui:  ContainerSkin.GRID_200.getSkin(),
               autoScroll: true,
               maxHeight: 550,
               scrollable: true,
      items:[
        Config(DateTimePropertyField, { bindTo: config.bindTo,
                                      labelSeparator:"",
                                      labelAlign: "top",
                                      fieldLabel:"",
    ...
    Config(Container, { style: "padding:8px;background-color:#c41313 !important;",
      items:[
      ...
      plugins:[
        Config(BindComponentsPlugin, {
          ...
          template: Config(DisplayField, { ui: ConfigUtils.asString( null), html: true, style: "color:#FFF;"
          })
        }),
        Config(BindVisibilityPlugin, { bindTo: this.getErrorMessagesExpression(),
