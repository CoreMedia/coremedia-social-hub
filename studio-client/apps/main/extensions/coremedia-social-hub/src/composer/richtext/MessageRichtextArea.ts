...
class MessageRichtextArea extends MessageRichtextAreaBase{
  ...
  plugins:[
    Config(BindPropertyPlugin, { bindTo: config.bindTo, bidirectional: true}),
    Config(ResizablePlugin, { fitComponent: true,
      resizableConfig: Config(StatefulResizer, { minHeight: 30,
                            handles: "s",
                            pinned: true,
                            dynamic: false,
                            embed: false,
