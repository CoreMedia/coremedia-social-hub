...
class DropContainer extends DropContainerBase{
  ...
  items:[
    Config(DisplayField, { value: config.property.getDisplayName(), ui:  DisplayFieldSkin.BOLD.getSkin()}),
    Config(Container, { ui:  ContainerSkin.FRAME_GRID_200.getSkin(),
      items:[
        Config(Container, { flex: 1,
          items:[
            Config(Container, { height: 30}),
            Config(Button, { text:  SocialHub_properties.upload_button_text,
          ...
          plugins:[
            Config(BindVisibilityPlugin, { bindTo: this.getItemsExpression(config.bindTo),
                                     transformer: (values:Array<any>):boolean =>   !values || values.length === 0 })
          ],
          layout: Config(VBoxLayout, { align: "center", pack: "center"
          })
        }),
        Config(Container, { flex: 1,
          items:[
            Config(Container, { flex: 1,
              items:[
                ...
                Config(AddItemButton, { uploadButtonHandler: bind(this,this.uploadButtonHandler),
                  plugins:[
                    Config(BindVisibilityPlugin, {
                            bindTo: this.getAddButtonVisibilityExpression(config.property, this.getItemsExpression(config.bindTo))})
                  ]
                })
              ],
              plugins:[
                Config(BindComponentsPlugin, {
                  ...
                  template: Config(DropItemThumbnail, { width: 130, height: 130
                  })
                })
              ],
  ...
  plugins:[
    Config(FileDropPlugin, { dropHandler: bind(this,this.handleFileDrop), customFileWrapperFactoryMethod: bind(this,this.createFileWrapper)})
  ],
  layout: Config(VBoxLayout, { align: "stretch"
  })
