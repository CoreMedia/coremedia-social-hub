...
class ComposeMessageJob extends RemoteJobBase implements BackgroundJob {
  ...
  getNameExpression(): ValueExpression {
    let msg = SocialHub_properties.compose_job_title;
    const network = SocialHub_properties[this.#adapterType.toLowerCase() + "_title"];
    msg = StringUtil.format(msg, network);
    return ValueExpressionFactory.createFromValue(msg);
  }

  getIconClsExpression(): ValueExpression {
    const cls = SocialHub_properties[this.#adapterType.toLowerCase()];
  ...
  getSuccessHandler(): AnyFunction {
    return (): void => {
      const socialHubAdapter = socialHubService.getAdapter(this.#adapterId);
