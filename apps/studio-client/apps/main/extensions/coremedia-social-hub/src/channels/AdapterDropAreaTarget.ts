...
class AdapterDropAreaTarget extends DropTarget {
  ...
  #allowDrop(dragInfo: DragInfo, fromView: Component): boolean {
    ...
    const adapter: SocialHubAdapter = this.#channelContainer.adapter;
    ...
    const contentContentType = SocialHubSettings_properties.social_hub_content_composer_document_type;
    const linkContentType = SocialHubSettings_properties.social_hub_content_composer_link_type;
    const contents = dragInfo.getContents();
  ...
  override notifyEnter(source: DragSource, e: Event, data: any): string {
    const dragInfo = DragInfo.makeDragInfo(data);
    const mayDrop = this.#allowDrop(dragInfo, data.view);
    if (mayDrop) {
      const linkContentType = SocialHubSettings_properties.social_hub_content_composer_link_type;
      const contentContentType = SocialHubSettings_properties.social_hub_content_composer_document_type;
  ...
  override notifyDrop(source: DragSource, e: Event, data: any): boolean {
    ...
    const composingType = this.#findDropType(e.target);
