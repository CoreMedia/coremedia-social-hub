...
class MessageImpl extends RemoteBeanImpl implements Message {
  ...
  getMessageContainerDescriptors():Array<any> {
    var dscrs:Array<any> = this.get(SocialHubPropertyNames.MESSAGE_CONTAINER_DESCRIPTORS);
    if(!this.#descriptors) {
      this.#descriptors = [];
      if(dscrs) {
        for(var descriptor of dscrs) {
          var d = new MessageContainerDescriptor(descriptor);
  ...
  deleteMessage(callback:AnyFunction):void {
    var deleteMethod = new RemoteServiceMethod(this.getUriPath(), "DELETE");
  ...
  retryMessage(callback:AnyFunction):void {
    var deleteMethod = new RemoteServiceMethod(this.getUriPath() + "/retry", "GET");
    deleteMethod.request({},
            (response:RemoteServiceMethodResponse):void => {
              callback.call(null);
            });
