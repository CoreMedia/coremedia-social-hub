...
class MediaItem {
  ...
  static fromContent(content: Content): MediaItem {
    ...
    if (content.isLoaded()) {
      ...
      const mediaType = socialHubService.getMediaType(content);
  ...
  load(callback?: AnyFunction): void {
    const this$ = this;
  ...
  render(): string {
    const thumbnailImage = editorContext._.getThumbnailImage(this.#values.content, ImageLinkListRenderer.DEFAULT_CROPPING);
