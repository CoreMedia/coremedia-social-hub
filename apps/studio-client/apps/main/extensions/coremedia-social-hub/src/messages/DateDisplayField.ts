...
interface DateDisplayFieldConfig extends Config<MessageDisplayFieldBase> {
}


class DateDisplayField extends MessageDisplayFieldBase{
  declare Config: DateDisplayFieldConfig;

  static override readonly xtype:string = "com.coremedia.blueprint.social.studio.config.message.field.date";

  constructor(config:Config<DateDisplayField> = null){
    super((()=> ConfigUtils.apply(Config(DateDisplayField, {
  ...
  items:[
    ...
    Config(DisplayField, { style: CustomStyles.READONLY_TEXT, ui: ConfigUtils.asString( null),
      plugins:[
        Config(BindPropertyPlugin, { transformer: EncodingUtil.encodeForHTML,
